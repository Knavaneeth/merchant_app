...
ons.ready(function() {
	...
	if (!empty(getStorage("merchant_token"))){
	    ...
	    showHomePage();	    			setTimeout(function(){getNewPendingOrders();},4000);
...
function createElement(elementId,elementvalue)
{
   ...
   if(elementId == 'con_devices'){
	   $('#con_devices').append("<ons-list-item>variableItem</ons-list-item>"); alert(ons.compile(content));
	}
	...
	success: function (data) {
		...
		if (data.code==1){
			...
			switch (action)
			{
				...
				case "GetTodaysOrder":
				notyAlert(data.msg,"error");
				$("#new-orders").html(' Sorry ! No Orders for the day ...');
				break;
...
function displayNewOrders(data,div_id)
{
	...
	var row_cnt=1;
	$.each( data, function( key, val ) {
		if (val.viewed==1){ //alert(val.new_order);
			if(row_cnt == 1)
			htm+='<ons-list-item modifier="tappable" onclick="viewOrder('+val.order_id+');">';
			else
			htm+='<ons-list-item modifier="tappable" onclick="viewOrder('+val.order_id+');" style="display:none;">';
		...
		var new_tag='';
			...
			new_tag='<div class="new-tag rounded">'+ getTrans('new','new') +'</div>';
			if(val.new_order != "1")
			new_tag='<div class="new-tag rounded">Pre-order</div>';

		htm+='<ons-col class="center" >';
		   htm+='<div class="delivery-icon">' + new_tag + " Order placed for the rate of <b>"+val.total_w_tax_prety+'</b>';
	...
	if(row_cnt==1){
		$(".list__item__center").hide();
		$(".main-home-tab").css('top','0px');clearTimeout(altpopup);
	}else{
		$(".main-home-tab").css('top','48px');
		playNewNotification();
	}
	//createElement('new-orders',htm);
...
function displayOrderDetails(data)
{
	...
	var html='<ons-list-header class="header">';
	 ...
	 else if ( data.status_raw=="delivered" || data.status_raw=="decline" || data.status_raw=="cancelled"){
     	$(".actions-1").hide();
     	$(".actions-2").hide();
     	$(".actions-3").hide();
     	$(".actions-4").show();
...
function playAudio(url) {
	...
	function playNewNotification(){ 
     ...
     my_media.play();
	 altpopup = setTimeout(function(){ playNewNotification();
	  },10000);
...
function displayDriverList(data)
{
	...
	if ( data.length>=1){
		$.each( data, function( key, val ) {
			 dump(val);
			 driver_name=val.driver_name;
			 html+='<ons-list-item modifier="tappable" onclick="setDriver('+val.id+', '+"'"+val.driver_name+"'"+' );" >';
             html+=driver_name;
             html+='</ons-list-item>';
		});
...
function color() {
 ...
 // $(".list__item__center").css('boxShadow','0px 0px 30px -5px rgba(' + red + ',' + green + ',' + blue + ',' + 1 + ')';
  $(".list__item__center").css('backgroundColor', 'rgba(' + red + ',' + green + ',' + blue + ',' + 1 + ')');
  hue++;
}
